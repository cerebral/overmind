import { Operator, createOperator, forEach, fork, map, mutate } from 'overmind'
...
export const updateState: () => Operator<StateMessage> = () =>
  mutate(({ state }, message) => {
    ...
    const key = path.pop()
    const target = path.reduce((aggr, key) => aggr[key].__CLASS__ ? aggr[key].value : aggr[key], app.state)
