...
type State = {
  ...
  hasActionsError: Derive<State, boolean>
  actionsCount: Derive<State, number>
...
const state: State = {
  ...
  hasActionsError: (state) => {
    if (!state.currentApp) {
      return false
    }

    return Object.values(state.currentApp.actions).some(
  ...
  componentsMounted: (state) => {
    ...
    return Object.keys(state.currentApp.components).reduce(
      (aggr, key) => {
  ...
  flushes: (state) => {
    ...
    return Object.keys(state.currentApp.flushes)
      .sort(
        (idA, idB) =>
  ...
  currentAction: (state) => {
    ...
    return state.currentApp.actions[state.currentApp.currentActionId]
  },
  currentChart: (state) => {
    if (!state.currentApp) {
      return null
    ...
    return state.currentApp.charts[state.currentApp.currentChartId]
  },
