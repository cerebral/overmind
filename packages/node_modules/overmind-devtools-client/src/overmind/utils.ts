...
export const runMutation = (state) => (mutation: Mutation) => {
  ...
  const key = pathArray.pop()
    ...
    const target = pathArray.reduce((current, pathKey) => current[pathKey] ? current[pathKey].__CLASS__ ? current[pathKey].value : current[pathKey] : current, state.__CLASS__ ? state.value : state)  
