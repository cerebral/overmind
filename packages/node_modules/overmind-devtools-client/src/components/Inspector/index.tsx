...
const PathKey: FunctionComponent<PathKeyProps> = ({
  ...
  return path.length ? (
    <span className={styles.key} onClick={disabled ? null : (event) => {
    ...
    if (selectedStatePath && path === selectedStatePath) {
      return (
        ...
        >
          ...
          <EditValue value={isClass ? value.value : value} onSubmit={onSubmitState} />
        </div>
      )
    }
      ...
      return (
        ...
        >
          ...
          <span className={styles.keyCount}>
            {isArray
              ? keys.length + ' items'
  : <span className={styles.inlineNested}>{isClass ? <span className={styles.inlineClass}>{value.name}</span> : null} {keys.sort().slice(0, 3).join(', ') + '...'}</span>}
          </span>
          {endBracket}
        </div>
      )
      ...
      <div>
        ...
        <div className={styles.nestedChildren}>
          {Array.isArray(value)
            ...
            : isClass ? [
              <span className={styles.otherValue} key={path.concat((path ? delimiter : '') + "__CLASS__")}>{value.name}</span>,
