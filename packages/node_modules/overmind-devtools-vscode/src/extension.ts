...
function getMarkup(scriptSource: vscode.Uri | string, port: number): string {
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('overmind-devtools.start', () => {
      const port: number =
        vscode.workspace.getConfiguration().get('overmind-devtools.port') || 3301
      ...
      DevtoolsPanel.markup = getMarkup(scriptFile, port)
