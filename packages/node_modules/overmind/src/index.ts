...
export type Pipe<Value = void, ReturnValue = any> = Operator<Value, ReturnValue>
...
export function fork<Input, Paths extends { [key: string]: Operator<Input> }>(
  operation: (input: Context<Input>) => keyof Paths,
  paths: Paths
): Operator<Input> {
  return (err, context, next) => {
    if (err) return next(err)
    paths[operation(context)](null, context, next)
  }
}

export function when<Input, Output = Input>(
  operation: (input: Context<Input>) => boolean,
  paths: {
    true: Operator<Input, Output>
    false: Operator<Input, Output>
  }
