...
function createContext(context, value, path?) {
  ...
  return {
    ...
    execution: {
      ...context.execution,
      operatorId: context.execution.operatorId + 1,
      path: path ? context.execution.path.concat(path) : context.execution.path,
    },
  }
}
...
>(
  ...
  const instance = (err, context, next) => {
    if (err) next(err)
      ...
      const newContext = createContext(context, context.value, path)
...
export function when<Input, OutputA, OutputB>(
  ...
  const instance = (err, context, next) => {
    if (err) next(err)
      ...
      if (operation(context)) {
        const newContext = createContext(context, context.value, 'true')
