  ...
  private createAction(name, action) {
    ...
    const actionFunc = async (value?) => {
      if (IS_PRODUCTION || action[IS_OPERATOR]) {
        ...
        const result = action(context, this.scopeValue(value, mutationTree))
...
export function map<Input, Output, ThisConfig extends Configuration = Config>(
  operation: (context: TContext<ThisConfig>, value: Input) => Output
  ...
  const instance = (err, context, next) => {
    if (err) next(err)
      ...
      const value = operation(context, context.value)
...
export function filter<Input, ThisConfig extends Configuration = Config>(
  operation: (context: TContext<ThisConfig>, value: Input) => boolean
): IOperator<ThisConfig, Input, Input> {
  const instance = (err, context, next, final) => {
    if (err) next(err)
    else {
      startDebugOperator('filter', operation, context)
      if (operation(context, context.value)) {
...
export function action<Input, ThisConfig extends Configuration = Config>(
  operation: (context: TContext<ThisConfig>, value: Input) => void
): IOperator<ThisConfig, Input, Input> {
  const instance = (err, context, next) => {
    if (err) next(err)
    else {
...
export function fork<
  ...
  ThisConfig extends Configuration = Config
>(
  operation: (context: TContext<ThisConfig>, value: Input) => keyof Paths,
  paths: Paths
): IOperator<ThisConfig, Input, Input> {
  const instance = (err, context, next) => {
    if (err) next(err)
      ...
      const path = operation(context, context.value)
