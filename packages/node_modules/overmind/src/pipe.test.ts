...
describe('PIPE', () => {
  ...
  describe('OPERATORS', () => {
    test('map', () => {
      expect.assertions(1)
      const test: Pipe<string, string> = pipe(
    ...
    test('when', () => {
      expect.assertions(1)
      const test: Pipe<string, string | number> = pipe(
        when(() => true, {
    ...
    test('wait', () => {
      ...
      const test: Pipe<string, string> = pipe(wait(500))
      return createMockAction(test)('foo').then(() => {
    ...
    test('run', () => {
      expect.assertions(1)
      const test: Pipe<string, string> = pipe(run(({ value }) => value + '!!!'))
      const action = createMockAction(test)
