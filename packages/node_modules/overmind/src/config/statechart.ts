...
function getInitialStates(charts: Statecharts, paths: Array<string[]> = [[]]) {
  ...
  Object.keys(charts).forEach((chartKey, index) => {
    ...
    if (chart.states[chart.initial].charts) {
      getInitialStates(chart.states[chart.initial].charts as Statecharts, paths)
    }
  })
...
type Match<T extends Statecharts> = {
  [I in keyof T]: {
    [S in keyof T[I]["states"]]: T[I]["states"][S]["charts"] extends Statecharts ? Match<T[I]["states"][S]["charts"]> : boolean
  }
}
