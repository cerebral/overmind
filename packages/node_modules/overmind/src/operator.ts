...
export function createTrackOperator<ThisConfig extends IConfiguration>(
  type: string,
  name: string,
  cb: (
    err: Error | null,
  ...
  const operator = (err, context, next, final) => {
    startDebugOperator(type, name, context)
    const trackStateTree = context.execution.getTrackStateTree()
    try {
      cb(
        err,
        {
          state: trackStateTree.state,
          effects: context.effects,
          actions: context.actions,
        },
        IS_PRODUCTION
          ? context.value
          : context.execution.scopeValue(context.value, trackStateTree),
        (err, value) => {
          stopDebugOperator(context, value)
          next(err, createContext(context, value))
        },
        ...
        trackStateTree.track.bind(trackStateTree)
      )
    } catch (error) {
      stopDebugOperator(context, context.value)
      next(error, createContext(context, context.value))
