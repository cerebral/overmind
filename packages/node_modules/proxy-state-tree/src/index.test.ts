...
describe('TrackStateAccessTree', () => {
  ...
  test('should allow tracking by flush', () => {
    ...
    const mutationTree = tree.getMutationTree().track()
...
describe('OBJECTS', () => {
  describe('ACCESS', () => {
    ...
    test('should access properties', () => {
      ...
      const tree = new ProxyStateTree(state)
      expect(tree.getTrackStateTree().state.foo.bar).toBe('baz')
    })
  ...
  describe('MUTATIONS', () => {
    test('should throw when mutating without tracking', () => {
      ...
      const tree = new ProxyStateTree(state)
      expect(() => {
        tree.getTrackStateTree().state.foo = 'bar2'
      }).toThrow()
    })
...
describe('ARRAYS', () => {
  describe('ACCESS', () => {
    ...
    test('should access properties', () => {
      ...
      const tree = new ProxyStateTree(state)
      expect(tree.getTrackStateTree().state.foo[0]).toBe('b')
    })
...
describe('FUNCTIONS', () => {
  ...
  test('should track state values from within derived', () => {
    ...
    const tree = new ProxyStateTree({
      ...
      foo: (proxyStateTree) => proxyStateTree.state.items.map((item) => item),
    })
    const accessTree = tree
      .getTrackStateTree()
      .track((mutations, paths, flushId) => {
    ...
    mutationTree.state.items[0].title = 'foo2'
    tree.flush()
    expect(reactionCount).toBe(1)
  })
})
...
describe('REACTIONS', () => {
  ...
  test('should allow subsequent mutation tracking and return all on flush', () => {
    ...
    const mutationTree = tree.getMutationTree().track()
    const mutationTree2 = tree.getMutationTree().track()
    mutationTree.state.foo = 'bar2'
