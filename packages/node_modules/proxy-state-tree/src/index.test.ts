...
describe('OBJECTS', () => {
  describe('ACCESS', () => {
    test('should create proxy when accessed', () => {
      ...
      const tree = new ProxyStateTree(state)
      const foo = tree.get().foo // eslint-disable-line
...
describe('ARRAYS', () => {
  ...
  test('should allow CONCAT of existing array', () => {
    ...
    expect(JSON.parse(JSON.stringify(tree.get().foo))).toEqual(['foo', 'bar'])
  })
})
...
describe('ITERATIONS', () => {
  ...
  test('should track new and deleted props on objects', () => {
    ...
    function update() {
      ...
      if (runCount === 1) {
        expect(paths).toEqual(new Set(['items']))
      }
      if (runCount === 2) {
        expect(paths).toEqual(new Set(['items']))
      }
...
describe('SCOPED', () => {
  it('should create a scoped tree instance', () => {
    const tree = new ProxyStateTree({
      foo: {
        bar: true,
      },
    ...
    expect((scoped as any).parentTree).toEqual(tree)
  })
  it('should share state and tracked mutations in scope affects trackers and listeners in base', () => {
    let listenerCount = 0
    const tree = new ProxyStateTree({
      foo: 'bar',
    })
    ...
    tree.addMutationListener(() => {
      listenerCount++
    })

    scoped.addMutationListener(() => {
      listenerCount++
    })
  ...
  it('should share state, but track mutations individually', () => {
    const tree = new ProxyStateTree({
      foo: 'bar',
    })
    ...
    scoped.startMutationTracking()
    scoped.get().foo = 'bar2'
    expect(() => (tree.get().foo = 'bar3')).toThrow()
    scoped.clearMutationTracking()
    expect(tree.get().foo).toBe('bar2')
    expect(scoped.get().foo).toBe('bar2')
  })
