    ...
    return new Proxy(target, {
      get: (obj, prop) => {
        ...
        if (utils.isStateMachine(namespaceObj)) {
          // Check if property exists in the StateMachine
          if (prop in namespaceObj) {
            const value = namespaceObj[prop]
            // If it's a method, bind it to the StateMachine instance
            if (typeof value === 'function') {
              return value.bind(namespaceObj)
            }
            return value
          }
          // Property doesn't exist in StateMachine, fall back to root
          return obj[prop]
        }
